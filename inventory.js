...
function findShoe(property, searchInput){
    let found = false; //adding this for exception handling
    ...
    if (!found) { //this is where the initial found status is used for exception handling
        throw new Error(`Could not find a shoe with such properties or property values`)
    } 
} 
...
function editShoeInformation(shoeArrayInstance, property, newProperty) {
    ...
    if (shoeArray.includes(shoeArrayInstance)  
    ...
    Bizzare. Please provie some ideas why that would ever happen?*/ 
        ...
        return console.log(`The ${property} was changed from ${oldProperty} to ${newProperty}`) 
